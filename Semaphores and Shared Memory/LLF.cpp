...
void scheduleTasks()
{
    //  Sort tasks in vector to be scheduled in LLF form (deadline - worst case), uses bubble sort.
    ...
    for (int i = 0; i < tasks.size(); i++)
    {
        ...
        int prevLaxity;
        if (command == "reserve") {
            prevLaxity = stoi(line) - reserve_time;
        }
        else if (command == "wait") {
            prevLaxity = stoi(line) - wait_time;
        }
        else if (command == "waitany") {
            prevLaxity = stoi(line) - waitany_time;
        }
        else if (command == "ticket") {
            prevLaxity = stoi(line) - ticket_time;
        }
        else if (command == "cancel") {
            prevLaxity = stoi(line) - cancel_time;
        }
        else{
            prevLaxity = stoi(line) - check_time;
        }
        ...
        for (int j = i + 1; j < tasks.size(); j++)
        {
            ...
            stringstream check(line);
            getline(check, line, ' ');
            string command2 = line;
            
            while (getline(check, line, ' ')){}
            int currLaxity;
            if (command2 == "reserve") {
                currLaxity = stoi(line) - reserve_time;
            }
            else if (command2 == "wait") {
                currLaxity = stoi(line) - wait_time;
            }
            else if (command2 == "waitany") {
                currLaxity = stoi(line) - waitany_time;
            }
            else if (command2 == "ticket") {
                currLaxity = stoi(line) - ticket_time;
            }
            else if (command2 == "cancel") {
                currLaxity = stoi(line) - cancel_time;
            }
            else{
                currLaxity = stoi(line) - check_time;
            }
            
            if (prevLaxity > currLaxity)
            {
