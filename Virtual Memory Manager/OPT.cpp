//
//
//  Created by Edgar Olvera on 11/24/20.
//
#include <iostream>
#include <stdio.h>
#include <stdlib.h>
...
void serviceRequests(int, int, int);
string serviceFaults(int, string, int, int);
...
int main(int argc, const char * argv[]) {
    ...
    {
        readRequests(myFile);
        serviceRequests(sid, page_number_length, lookahead_window_or_X);
        cout << endl;
...
void serviceRequests(int sid, int page_number_length, int look_ahead)
{
    ...
    while (!requests.empty())
    {
        ...
        //  If page is not in our main memory we must invoke page fault
        if (find(frames.begin(), frames.end(), page_number) == frames.end())
        {
            page_faults++;
            
            //  does our page replacement algorithm
            string switch_page = serviceFaults(sid, page_number, page_number_length, look_ahead);
            
            //  We will place the memory requested into our main memory and swap any unused page back to disk.
...
string serviceFaults(int sid, string page_number, int page_length, int look_ahead)
{
    ...
    {
        //  else we use OPT - X page replacement so, we remove the page that is X farthest of being requested

        int i{};
        vector<string>::iterator farthest_page_request = frames.begin();
        ...
        for (auto req : requests)
        {
            ...
            string page = req.substr(0, page_length);
            
            vector<string>::iterator it = find(frames.begin(), frames.end(), page);
            
            if (it != frames.end())
            {
                farthest_page_request = it;
                switch_page = page;
            }
        ...
        frames.erase(farthest_page_request);
