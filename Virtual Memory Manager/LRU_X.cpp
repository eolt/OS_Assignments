...
string serviceFaults(int, string, int);
...
void serviceRequests(int sid, int page_number_length, int x)
{
    ...
    while (!requests.empty())
    {
        ...
        vector<string>::iterator it = find(frames.begin(), frames.end(), page_number);
        ...
        if (it == frames.end())
        {
            ...
            string switch_page = serviceFaults(sid, page_number, x);
...
string serviceFaults(int sid, string page_number, int x)
{
    ...
    {
        //  else we use LRU - X page replacement so, we remove the the X page that was in our memory
        switch_page = frames.at(x - 1);
        frames.erase(frames.begin() + (x - 1));
        frames.push_back(page_number);
    }
